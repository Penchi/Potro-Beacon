...
public class AlumnoActivity extends Activity implements EasyPermissions.PermissionCallbacks, BeaconConsumer, RangeNotifier {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tv = (TextView) findViewById(R.id.txtConsola);
    ...
    //Se busca si estamos dentro de una clase y actuamos si lo estamos
    public void VericiarExistencia(final int id)
    {
        ...
        referencia.addValueEventListener(new ValueEventListener()
        {
            ...
            public void onDataChange(DataSnapshot dataSnapshot)
            {
                try {
                    ...
                    tv.setText(tv.getText() + "id Salon:" + salon.getId() + "\n");
                    tv.setText(tv.getText() + "Número de clases:" + salon.getClases().size() + "\n");

                    ObjectMapper mapper = new ObjectMapper();
                    Map<String, Object> mapClases = salon.getClases();
                    Clase[] clases = new Clase[mapClases.size() - 1];

                    Map<String, Object> map = mapClases;
                    for (Map.Entry<String, Object> entry : map.entrySet()) {
                        //System.out.println(entry.getKey() + "/" + entry.getValue());
                        //Clase clase = Clase.class.cast(entry.getValue());
                        try {
                            Clase clase = mapper.convertValue(entry.getValue(), Clase.class);

                            tv.setText(tv.getText() + "id de clase:" + clase.getId() + "\n");
                            tv.setText(tv.getText() + "Nombre de clase:" + clase.getNombre() + "\n");
                            tv.setText(tv.getText() + "entrada:" + clase.getEntrada() + "\n");
                            tv.setText(tv.getText() + "salida:" + clase.getSalida() + "\n");
                            tv.setText(tv.getText() + "SpreadSheetId:" + clase.getSpreadSheetId() + "\n");
                        ...
                        {
                            tv.setText("NO SE ENCONTRÓ EL BEACON EN FIREBASE");
                        }
                    }
                }
                catch(Exception ex)
    ...
    public void RegistrarAsistencia(String spreadSheetId)
    {
        ...
        if(!ocupado)
        {
            ...
            if (beacons.size() > 0)
            {
                ...
                for (int i = 0; i < tamano; i++)
                    if (listaBeacons[i].getManufacturer() == idBeaconConectado) {
                        ocupado = false;
                        return;
                    }
                idBeaconConectado = -1;
                ...
                for (int i = 0; i < tamano; i++)
                {
                    tv.setText(tv.getText().toString() + "Distancia en metros:" + listaBeacons[i].getDistance() + "\n");
                    tv.setText(tv.getText().toString() + "Dirección Bluetooth:" + listaBeacons[i].getBluetoothAddress() + "\n");
                    tv.setText(tv.getText().toString() + "UUID:" + listaBeacons[i].getId1() + "\n");
                    tv.setText(tv.getText().toString() + "ID de fabricante:" + listaBeacons[i].getManufacturer() + "\n");
                    VericiarExistencia(listaBeacons[i].getManufacturer());
                }
            }
            else
            {
    ...
    private void getResultsFromApi() {
        ...
        {
            new AlumnoActivity.MakeRequestTask(mCredential).execute();
        }
    }
    ...
    private class MakeRequestTask extends AsyncTask<Void, Void, List<String>>
    {
         ...
         * @throws IOException
         */
        private List<String> AnotarAsistencia() throws IOException
        {
            System.out.println("ANOTANDO ASISTENCIA");
            String range = "a1:z1";
            ...
            if (values != null)
            {
                ...
                {
                    if((values.get(0).get(i) + "").equalsIgnoreCase(nombre))
                    {
                        String letra = "" + ((char) (i + 97));
                        Calendar cal = Calendar.getInstance();
                        cal.setTime(new Date());
                        int dia = cal.get(Calendar.DAY_OF_MONTH) + 1;
                        EscribirSheet(letra+dia, "X");
                        return null;
                    }
                }
                String letra = "" + ((char) (i + 97));
                EscribirSheet(letra + "1", nombre);
                Calendar cal = Calendar.getInstance();
                cal.setTime(new Date());
                int dia = cal.get(Calendar.DAY_OF_MONTH) + 1;
                EscribirSheet(letra + dia, "X");
